...
| 95 | Dedupe hourly excluded-range escalation/malformed-CSV warnings — `_warn_once()` in `ha_data.py` fires WARNING once per condition per AppDaemon process lifetime via `self._excluded_range_warned`, then INFO on repeats; resets on restart | v0.12.0-alpha-11 |
