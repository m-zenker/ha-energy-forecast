...
- `apps/energy_forecast/energy_forecast.py` — `_retrain()` called `split_ev_charging_from_sensor()` directly when `ev_charging_sensor` was configured. That function fills rows predating the sensor's first record with 0 kWh, so real EV sessions from before the wallbox was installed were silently treated as normal household load and included in training rather than excluded. Fixed by switching to `split_ev_charging_hybrid()`: sensor readings are used inside the wallbox sensor's history window; threshold detection covers the earlier period.
- `apps/energy_forecast/energy_forecast.py` — `_retrain()` applied ±1h adjacent-hour padding (to absorb charger ramp-up/down noise around threshold-detected EV hours) to every EV hour, including hours with exact wallbox sensor readings. Sensor-detected hours need no padding and discarding them is wasteful; for long solar-surplus charging sessions spanning most of the day, the extra drops pushed days below regime clustering's 18 h/day completeness floor, silently removing them from cluster training entirely rather than counting them as EV-excluded (6 of 8 post-wallbox EV days were affected). The padding is now restricted to threshold-detected hours only (hours outside the sensor's coverage window).
...
- `apps/energy_forecast/ha_data.py`, `apps/energy_forecast/energy_history_backfill.py`, `apps/energy_forecast/model.py` — Hourly grid-import diffs of exactly `0.0` — possible once solar can fully cover household load — were silently excluded from `energy_history.csv` and from model training because all four filter sites used a strict `> 0` bound (`_raw_to_kwh_diff()`, `validate_energy_cache()`, the backfill tool's diff step, and `EnergyForecastModel.train()`). Those real zero-import hours formed genuine gaps in the cache, starving `lag_24h` features of history and producing NaN warnings each hourly cycle. Fixed by widening the accepted range to `>= 0` at all four sites. A companion fix ensures meter resets are still dropped rather than kept as fabricated `gross_kwh=0` rows: a negative raw diff that `clip(lower=0)` had previously made indistinguishable from a real zero reading is now filtered on the pre-clip raw diff directly, so only a diff that is truly `0` produces a valid zero-consumption cache entry.
- `apps/energy_forecast/energy_forecast.py` — `_retrain()` and `_update_sensors()` both ran EV threshold detection (`ha_data.split_ev_charging`) on raw grid-import `gross_kwh` *before* the solar/grid-export/battery target correction (`_apply_target_correction`) was applied. Once solar can cover part of a charging session's load, raw grid import for that hour can fall below `ev_charging_threshold_kwh` (default 7 kWh/h) even though corrected total consumption is well above it. This caused two silent failures: `binary_sensor.ha_energy_forecast_unusual_consumption` fired spuriously during legitimate solar-covered EV charging, and those same hours trained into the model as normal baseline household load instead of being excluded. Fixed by moving `_apply_target_correction` before `split_ev_charging` in both methods. In `_update_sensors`, `recent_actuals` is now derived from the already-corrected `full_actuals` via the EV split, so the now-redundant second correction call on `recent_actuals` has been removed.
- `apps/energy_forecast/const.py` — `resolve_timezone()` now coerces `ha_timezone` to `str` before comparing or returning it. AppDaemon's `get_timezone()` is type-hinted as `-> str`, but the installed runtime validates `AppDaemonConfig.time_zone` through `pytz.timezone(...)` and returns a `pytz.tzinfo` object instead of a plain string when no explicit `timezone:` override is set in `apps.yaml`. That object flowed straight into `urllib.parse.quote()` in `weather.py`, crashing every `_retrain()` call with `TypeError: quote_from_bytes() expected bytes` and leaving the model stuck on stale data — reported as "forecast keeps climbing" in GitHub Discussion #15. As a side effect, a spurious "configured timezone doesn't match HA's" startup WARNING that fired on every launch is also fixed: the old `tzinfo_object != str` comparison always evaluated `True` across types, even when both represented the same zone.
- `apps/energy_forecast/weather.py` — `fetch_historical_weather()` and `fetch_open_meteo()` now coerce their `timezone` argument to `str` before passing it to `urllib.parse.quote()`. Defense-in-depth for the same Discussion #15 crash: both functions are public and independently callable, so they should not rely on callers having gone through `resolve_timezone()` first.
- `apps/energy_forecast/ha_data.py` — `_raw_to_kwh_diff()` gains an opt-in `end_time` parameter that extends the resampled series through that timestamp (via `reindex`) before forward-filling. Without it, `resample().last()` stopped generating bins at the last actual state received from HA — so a `total_increasing` cumulative meter that stops emitting updates entirely (e.g. a grid-import meter while solar is covering 100% of household load) produced no rows at all for those silent hours instead of the genuine `0.0`-kWh diffs they represent. All four fetch call sites — `fetch_recent_energy()`, `fetch_energy_history()`, `fetch_energy_history_15m()`, and `fetch_recent_energy_15m()` — now pass `end_time=pd.Timestamp.now(tz=timezone)`, backfilling trailing sensor silence through the current time. Fixes the recurring "insufficient lag features" / `lag_24h has N/48 NaN` WARNING that fired each hourly cycle during normal solar-covered operation.
- `ha_appdaemon_config.yaml` — `system_packages` now includes `g++` explicitly. Alpine 3.23 (shipping in HA OS 18.1) split the C++ compiler out of the `build-base` metapackage, so `lightgbm`'s CMake build failed with `Could not find the compiler specified in the environment variable CXX: c++` and AppDaemon never started (4 consecutive restart-loop failures observed 2026-07-21). `README.md` and the local dev/test environment's `system_packages` guidance updated to match.
- `apps/energy_forecast/model.py` — the three `LGBMRegressor.fit()` call sites in `train()`'s CV loop used the deprecated `eval_set=[(X_val, y_val)]` kwarg, which LightGBM ≥4.7.0 flags with `LGBMDeprecationWarning` (observed in production logs 2026-07-21) ahead of removal in a future major release. Migrated to the replacement `eval_X=X_val, eval_y=y_val` kwargs — passed directly, not wrapped in a list; LightGBM's `_validate_eval_set_Xy` only accepts a **tuple** for multiple eval sets, and wrapping a single set in a list raised `TypeError: Data list can only be of ndarray or Sequence`. `ha_appdaemon_config.yaml`'s `init_commands` now pins `lightgbm>=4.7.0,<5.0` (previously unpinned, which is how a breaking-API version silently reached production) — floor set to the version the new fit API requires, ceiling to block a future major bump from doing the same thing again unnoticed.
...
### Added
- `apps/energy_forecast/energy_forecast.py` — When quantile models are trained, publishes 32 new sensors (`sensor.energy_forecast_{today|tomorrow}_{slot}_10th_pct` / `_90th_pct`) for all 8 three-hour slots in each day. Enables ha-energy-manager's battery manager to use pessimistic/optimistic per-slot consumption estimates for overnight planning. MQTT discovery for these sensors is registered lazily on first interval publish to avoid orphan entities when quantile models are absent. ([cross-repo gap A])
...
- `apps/energy_forecast/energy_forecast.py` — `predict_scenario()` now accepts a `room_areas` parameter and forwards it through `_get_scenario_cb`, so the scenario baseline uses the same area-weighted thermal pressure as the live published forecast. Previously the scenario call always used the default equal-area weights, causing subtle divergence between scenario deltas and actual forecasts on multi-room configurations.
- `apps/energy_forecast/energy_forecast.py` — `_publish_scenario_forecast` now routes through MQTT Discovery when `mqtt_discovery=True`. All 11 scenario sensors are lazily registered on first publish and then updated via `_mqtt_set_sensor`. Scenario entity IDs are also added to `_cleanup_legacy_states` so no ghost `set_state` entities persist after switching to MQTT mode.
- `apps/energy_forecast/clustering.py` — `_strip_partial_last_day()` now filters the last date from clustering input when it has fewer than 24 complete hours. Retrains firing between 18:xx–23:xx no longer produce a bfill-interpolated column 23 in the KMeans pivot, which was distorting late-evening centroids and degrading regime classification for evening-heavy consumption profiles. (#86)
- `scripts/deploy.py` — removed unused `repo_data` variable; narrowed the `_ensure_dir` exception handler from bare `Exception` to `OperationFailure` (pysmb) so network-level errors during directory creation propagate instead of being silently swallowed.
- `.gitignore` — added `htmlcov/`, `.coverage`, and `work/` entries; changed `.ipynb_checkpoints` to the recursive `**/.ipynb_checkpoints/` pattern so nested checkpoint directories are also ignored. Coverage config updated to omit checkpoint directories from measurement.
...
- `apps/energy_forecast/weather.py` — `numpy`, `pandas`, and `requests` imports are now deferred to function bodies, matching the established lazy-import pattern used in `model.py`, `ha_data.py`, and `clustering.py`. Reduces AppDaemon startup time when the module is imported but weather fetching is not immediately triggered.
