...
- `apps/energy_forecast/ha_data.py` — two new helpers support hybrid EV-detection when a
  ...
  household load.
- `apps/energy_forecast/energy_forecast.py` — `heating_season` attribute added to `sensor.energy_forecast_today` showing the active on/off thresholds, their source (learned/config/default), the label source (meter/entity/none), the number of labelled days used for learning, and the mismatch rate. Visible under the sensor's attributes in the HA developer tools.
- `docs/examples/heating_season_automation.yaml` — example HA automation for a stable daily heating-season switch: uses the daily-mean outdoor temperature forecast, enforces a 48 h minimum dwell time to avoid toggling on unsettled weather, and includes a frost-override that forces heating on below 4 °C regardless of dwell state.
- `scripts/backtest_heating_feature.py` — gitignored offline analysis script comparing heating-feature variants on historical data; meter label paired with the daily-mean rule cut day-ahead MAE by 5 % for September 2026 against the old hourly hysteresis projection.
...
- `apps/energy_forecast/energy_forecast.py` — `heating_active` was projected ON for every forecast hour below `heating_temp_on` using per-hour outdoor temperatures, ignoring the real heating switch. On cold autumn nights with heating still off, the model predicted winter heat-pump load — the root cause of the live MAE rise from mid-September 2026 (mean bias ≈ +0.3 kWh/h below 12 °C). The projection now makes a single daily decision from the forecast daily mean temperature and holds that state for the full calendar day.
- `apps/energy_forecast/energy_forecast.py` — installations with `heating_system_active_entity` but no `climate_entities` never entered the heating-active projection path, so the model predicted with `heating_active=1` while training used the real switch. The projection no longer requires climate entities to be configured.

### Changed
- `apps/energy_forecast/energy_forecast.py`, `apps/energy_forecast/heating_season.py` — `heating_temp_on` / `heating_temp_off` are now optional overrides of daily-mean thresholds (previously hourly outdoor temps, defaults 14/18 °C). They must be set together; one alone is ignored with a warning. When unset, thresholds are learned per installation at each retrain from historical data (fallback 12/16 °C until ≥60 labelled days with ≥4 season transitions exist).
- `apps/energy_forecast/model.py`, `apps/energy_forecast/energy_forecast.py` — when `physics.heating_sub_meter_sensor` is configured, the `heating_active` training feature now uses the sub-meter "heating day" label (>0.5 kWh space heating per day) rather than the raw switch state. The switch is still used for τ and physics calibration.
...
- `apps/energy_forecast/ha_data.py`, `apps/energy_forecast/energy_history_backfill.py`, `apps/energy_forecast/model.py` — Hourly grid-import diffs of exactly `0.0` — possible once solar can fully cover household load — were silently excluded from `energy_history.csv` and from model training because all four filter sites used a strict `> 0` bound (`_raw_to_kwh_diff()`, `validate_energy_cache()`, the backfill tool's diff step, and `EnergyForecastModel.train()`). From solar commissioning (2026-07-16) onwards, those real zero-import hours formed genuine gaps in the cache, starving `lag_24h` features of history and producing NaN warnings each hourly cycle; this is also the suspected root cause of the previously-logged "actuals_history freeze." Fixed by widening the accepted range to `>= 0` at all four sites. A companion fix (found in code review) ensures meter resets are still dropped rather than kept as fabricated `gross_kwh=0` rows: a negative raw diff that `clip(lower=0)` had previously made indistinguishable from a real zero reading is now filtered on the pre-clip raw diff directly, so only a diff that is truly `0` produces a valid zero-consumption cache entry.
- `apps/energy_forecast/energy_forecast.py` — `_retrain()` and `_update_sensors()` both ran EV threshold detection (`ha_data.split_ev_charging`) on raw grid-import `gross_kwh` *before* the solar/grid-export/battery target correction (`_apply_target_correction`) was applied. Once solar can cover part of a charging session's load, raw grid import for that hour can fall below `ev_charging_threshold_kwh` (default 7 kWh/h) even though corrected total consumption is well above it — observed 2026-07-17 with 0.5 kWh raw import against ~8.6 kWh corrected consumption during a midday charge session. This caused two silent failures: `binary_sensor.ha_energy_forecast_unusual_consumption` fired spuriously during legitimate solar-covered EV charging, and those same hours trained into the model as normal baseline household load instead of being excluded, biasing forecasts upward as more solar-covered sessions accumulate. Fixed by moving `_apply_target_correction` before `split_ev_charging` in both methods. In `_update_sensors`, `recent_actuals` is now derived from the already-corrected `full_actuals` via the EV split, so the now-redundant second correction call on `recent_actuals` has been removed.
- `apps/energy_forecast/ha_data.py` — `_raw_to_kwh_diff()` gains an opt-in `end_time` parameter that extends the resampled series through that timestamp (via `reindex`) before forward-filling. Without it, `resample().last()` stopped generating bins at the last actual state received from HA — so a `total_increasing` cumulative meter that stops emitting updates entirely (e.g. a grid-import meter while solar is covering 100% of household load, which has nothing new to report) produced no rows at all for those silent hours instead of the genuine `0.0`-kWh diffs they represent. All four fetch call sites — `fetch_recent_energy()`, `fetch_energy_history()`, `fetch_energy_history_15m()`, and `fetch_recent_energy_15m()` — now pass `end_time=pd.Timestamp.now(tz=timezone)`, backfilling trailing sensor silence through the current time. Fixes the recurring "insufficient lag features" / `lag_24h has N/48 NaN` WARNING that fired each hourly cycle during normal solar-covered operation, where those dropped hours caused `lag_24h`/`lag_168h` features to fall back to training medians instead of reflecting the real (zero) consumption.
